  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-14">
      ...
      <p className="mt-4 max-w-prose text-muted">
        This service helps me check whether a claim is valid, update a probability when new
        evidence arrives, and read the expected payoff of a yes-or-no business outcome.
      </p>
        ...
        {syllogism ? (
          <div className="mt-3 max-w-prose">
            ...
            <p className="mt-2 max-w-prose text-sm text-muted">
              I ignore how convincing the story sounds and look only at the argument shape. An
              if-then rule only licenses two moves: seeing the if-part (valid) or seeing the
              then-part is false (also valid). Seeing the then-part alone, or denying the
              if-part, looks persuasive in marketing language but proves nothing.
            </p>
          </div>
        ) : (
      ...
      <section className="mt-12">
        ...
        {plausibility ? (
          <div className="mt-3">
            ...
            <p className="mt-2 max-w-prose text-sm text-muted">
              For a manager: given that a case was flagged, this posterior is the chance it is
              actually the rare condition — most flags can still be false alarms when the base
              rate is low.
            </p>
          </div>
        ) : (
      ...
      <section className="mt-12">
        ...
        {bernoulli ? (
          <div className="mt-3">
            ...
            <p className="mt-2 max-w-prose text-sm text-muted">
              Expected payoff is the long-run average if I repeated this bet many times — it is
              not a promise about what happens on the next single try.
            </p>
          </div>
        ) : (
